...
class OrdinaryForestSearchTimersEnergyTextsTest(unittest.TestCase):
    def make_player_and_storage(self, *, energy: int = 100):
        ...
        player = create_player(
            game_id=game_id,
            platform="telegram",
            external_user_id="889",
            name="Лесной проверяющий",
            race_id="human",
            races=races,
        )
        ...
        storage.save_new_player(player, "telegram", "889")
        return storage, storage.get_player_by_platform("telegram", "889")

    def test_ordinary_forest_search_rules_are_explicit_in_config_and_location_text(self):
    ...
    def test_full_energy_search_spends_two_energy_and_uses_thirty_second_timer(self):
        ...
        updated = storage.get_player_by_platform("telegram", "889")
        timer = updated.get("active_timer") or {}
        self.assertIn("Поиск начался", response.text)
        self.assertIn("Локация: Обыкновенный лес", response.text)
        ...
        self.assertIn("Жизни:", response.text)
        ...
        self.assertEqual("ordinary_forest", timer.get("location_id"))
    ...
    def test_timer_check_and_completion_keep_search_text_and_location(self):
        ...
        player = storage.get_player_by_platform("telegram", "889")
        player["active_timer"]["event"] = {
            "type": "dry_tree",
            "location_id": "ordinary_forest",
            "text": "🪵 Вы обнаружили сухое дерево.",
            "event_id": "ordinary_forest_test_event",
        }
        ...
        self.assertIn("Локация: Обыкновенный лес", completed.text)
        self.assertNotIn("waterside", completed.text.casefold())
        updated = storage.get_player_by_platform("telegram", "889")
        self.assertIsNone(updated.get("active_timer"))
        self.assertEqual("ordinary_forest", updated.get("current_location"))
    ...
    def test_back_cancels_active_search_without_returning_city_buttons(self):
        ...
        updated = storage.get_player_by_platform("telegram", "889")
        self.assertIsNone(updated.get("active_timer"))
        self.assertEqual("ordinary_forest", updated.get("current_zone"))
