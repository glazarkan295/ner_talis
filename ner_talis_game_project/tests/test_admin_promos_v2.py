...
class AdminPromosV2Test(unittest.TestCase):
    ...
    def test_create_list_delete_promo(self):
        ...
        self.assertEqual(self.client.request("DELETE", "/api/admin/v2/promos?code=START100", headers=self._auth(token)).status_code,422)
        deleted = self.client.request("DELETE", "/api/admin/v2/promos?code=START100&confirm=START100", headers=self._auth(token))
