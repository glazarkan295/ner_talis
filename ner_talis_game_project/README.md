...
- выход из города через городские ворота
- стартовая внешняя локация «Холмистые луга»
- поиск событий с расходом энергии
- события: ингредиенты, камень/руда, ягоды, ловушка, «Блик в траве», заготовка битвы
- лагерь, готовка и употребление базовых лагерных блюд
- безопасная внешняя зона «Крепость в ущелье» с отдельным меню
...
Через `Городские ворота → Выход из города` игрок попадает к выбору внешних направлений. Сейчас подключены «Холмистые луга» и «Крепость в ущелье».
...
Он содержит расы, Селдар, стартовую внешнюю локацию `hilly_meadows` и пустую схему хранилища.
...
Smoke-test проверяет создание персонажа, уникальность имени, привязку Telegram/VK к одному `game_id`, сохранение в SQLite между перезапусками, вход в город Селдар, выход к Холмистым лугам, поиск/лагерь и то, что общий запуск вызывает оба бота.
