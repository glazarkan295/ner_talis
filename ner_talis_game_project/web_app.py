...
def create_app() -> FastAPI:
    app = FastAPI(title="Ner-Talis", version="0.3.0")
    ...
    def get_session_and_player_by_token(token: str, scope: str) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
        st = storage()
        if hasattr(st, "get_player_by_web_token"):
            player, session = st.get_player_by_web_token(token, scope=scope)
    ...
    def get_player_by_public_id(public_id: str) -> dict[str, Any] | None:
        st = storage()
        if hasattr(st, "get_player_by_public_id"):
            return st.get_player_by_public_id(public_id)
        for player in (st.load().get("players") or {}).values():
            if isinstance(player, dict) and str(player.get("public_id")) == str(public_id):
                return player
        return None

    def render_profile_by_token(token: str) -> str:
        player, session = get_session_and_player_by_token(token, PROFILE_SCOPE)
    ...
    def render_profile_by_identifier(identifier: str) -> str:
        player, session = get_session_and_player_by_token(identifier, PROFILE_SCOPE)
        if player is not None:
            return _render_profile_html(player, session)
        player = get_player_by_public_id(identifier)
        if player is not None:
    ...
    def profile_page_path(identifier: str) -> str:
        if len(identifier) < 8:
            raise HTTPException(status_code=400, detail="Некорректная ссылка профиля.")
        return render_profile_by_identifier(identifier)
    ...
    def render_pavilion_by_token(token: str) -> str:
        player, session = get_session_and_player_by_token(token, PAVILION_SCOPE)
        if player is None or session is None:
