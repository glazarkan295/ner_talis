...
- `ner_talis_game_project/admin_world_api.py` — API **Конструктора мира V2** (/api/admin/v2/world/...): генерик над kind (location, mob, button, transition, event, npc, quest). list/get/create(world.create_draft)/edit(world.edit_draft)/status/validate(world.validate)/publish(world.publish, опасное, валидирует перед публикацией)/disable/archive — каждая мутация через run_admin_operation. Опирается на world_content_registry.
...
- `world_content_registry.py` — **Конструктор мира V2**: генерик-реестр data-driven контента (kind→id→envelope) с жизненным циклом draft→review→ready→published→disabled→archived, переходами статусов, валидацией (VALIDATORS по kind: location/mob/button/transition/event/npc/quest — моб: боевые поля + таблица дропа; button/transition: cross-object проверка локаций + тупик через location_has_exit; event: локация+дроп-предметы+battle_mob существуют, тип/результат из справочников; npc: функции из NPC_FUNCTIONS; quest: цель по goal_type ссылается на существующий предмет/моба/локацию/NPC) и хранилищем JSON с блокировкой (env WORLD_CONTENT_PATH, по умолч. data/world_content.json). Чистый слой данных; аудит/права — в роутере.
