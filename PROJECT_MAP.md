...
- `ner_talis_game_project/admin_world_api.py` — API **Конструктора мира V2** (/api/admin/v2/world/...): генерик над kind (location, mob). list/get/create(world.create_draft)/edit(world.edit_draft)/status/validate(world.validate)/publish(world.publish, опасное, валидирует перед публикацией)/disable/archive — каждая мутация через run_admin_operation. Опирается на world_content_registry.
...
- `world_content_registry.py` — **Конструктор мира V2**: генерик-реестр data-driven контента (kind→id→envelope) с жизненным циклом draft→review→ready→published→disabled→archived, переходами статусов, валидацией (VALIDATORS по kind: location, mob — у моба проверка боевых полей + таблицы дропа: item_id существует в каталоге, шанс 0–100, min≤max, экономич. пороги) и хранилищем JSON с блокировкой (env WORLD_CONTENT_PATH, по умолч. data/world_content.json). Чистый слой данных; аудит/права — в роутере.
