...
- `ner_talis_game_project/admin_world_api.py` — API **Конструктора мира V2** (/api/admin/v2/world/...): генерик над kind (location, mob, button, transition, event, npc, quest, raid) + /preview (world.view) и /test-run (world.test_run). list/get/create(world.create_draft)/edit(world.edit_draft)/status/validate(world.validate)/publish(world.publish, опасное, валидирует перед публикацией)/disable/archive — каждая мутация через run_admin_operation. Опирается на world_content_registry.
